...
def gen_p2p_028():
    ...
    sample = random.sample(above, min(3, len(above))) + random.sample(below, min(2, len(below)))
...
def gen_itg_015():
    ...
    sample = random.sample(term_users, min(3, len(term_users))) + random.sample(priv_users, min(2, len(priv_users)))
...
def gen_fin_042():
    ...
    # --- 5 hand-crafted sample items with known outcomes ---
    # Item 1: PASS on all attributes (clean, above threshold, proper approval)
    # Item 2: FAIL on C (self-approval violation)
    # Item 3: PASS on all (below threshold so B is N/A, clean)
    # Item 4: FAIL on E (wrong accounting period)
    # Item 5: FAIL on D (missing supporting docs)
    sample = [
        {
            "JE_Number": "JE-2024-0015",
            "Posting_Date": "15/01/2024",
            "Account": "4100-Sales Revenue",
            "Description": "Manual adjustment - accrual for Q4 revenue",
            "Amount_GBP": 245000.00,
            "Preparer": "Alice Brown",
            "Approver": "Sarah Mitchell",
            "Period": "P01",
            "Supporting_Doc": "Accrual schedule ref ACC-2024-015",
        },
        {
            "JE_Number": "JE-2024-0043",
            "Posting_Date": "20/03/2024",
            "Account": "6100-SGA Expense",
            "Description": "Manual adjustment - reclassification of marketing costs",
            "Amount_GBP": 187500.00,
            "Preparer": "Bob Carter",
            "Approver": "Bob Carter",
            "Period": "P03",
            "Supporting_Doc": "Reclass memo ref RCL-2024-043",
        },
        {
            "JE_Number": "JE-2024-0078",
            "Posting_Date": "10/06/2024",
            "Account": "5200-COGS",
            "Description": "Manual adjustment - inventory provision",
            "Amount_GBP": 42300.00,
            "Preparer": "Carol Davis",
            "Approver": "James Wright",
            "Period": "P06",
            "Supporting_Doc": "Inventory count sheet ref INV-2024-078",
        },
        {
            "JE_Number": "JE-2024-0112",
            "Posting_Date": "05/10/2024",
            "Account": "7200-R&D Expense",
            "Description": "Manual adjustment - correction of misposted R&D costs",
            "Amount_GBP": 156000.00,
            "Preparer": "David Evans",
            "Approver": "Emily Foster",
            "Period": "P09",
            "Supporting_Doc": "Correction memo ref COR-2024-112",
        },
        {
            "JE_Number": "JE-2024-0145",
            "Posting_Date": "28/11/2024",
            "Account": "8100-Other Income",
            "Description": "Manual adjustment - intercompany settlement",
            "Amount_GBP": 320000.00,
            "Preparer": "Alice Brown",
            "Approver": "James Wright",
            "Period": "P11",
            "Supporting_Doc": "",
    ...
    engagement = {
        ...
        "testing_attributes": [
            ...
            {"ref": "C", "name": "Dual authorization verified — preparer and approver are different individuals", "applies_to": "all"},
            {"ref": "D", "name": "Supporting documentation is attached and referenced", "applies_to": "all"},
            {"ref": "E", "name": "Journal entry posted within correct accounting period (within 5 days of period end)", "applies_to": "all"},
        ],
        "instructions": (
            "Execute fieldwork for Control FIN-042. There are 5 sample items. "
            "Verify: (A) the JE listing is complete at control level, (B) above-threshold "
            "entries (>GBP 100K) have Finance Director sign-off, (C) each entry has dual "
            "authorization (preparer != approver), (D) supporting docs are attached, and "
            "(E) entries are posted in the correct period. Review the approval emails and "
            "JE listing PDF for evidence. The policy document defines the rules."
        ),
        "evidence_files": [
            {"path": "evidence/je_listing.pdf", "type": "pdf", "focus": "journal_entry_listing_and_detail"},
            {"path": "evidence/approval_email_001.eml", "type": "email", "focus": "authorization_for_JE-2024-0015"},
            {"path": "evidence/approval_email_002.eml", "type": "email", "focus": "authorization_for_JE-2024-0043_SELF_APPROVAL"},
            {"path": "evidence/approval_email_003.eml", "type": "email", "focus": "authorization_for_JE-2024-0112"},
            {"path": "evidence/je_policy.pdf", "type": "pdf", "focus": "policy_rules_and_thresholds"},
        ],
    }
    (proj / "engagement.json").write_text(json.dumps(engagement, indent=2))

    # --- Full population (background, not tested) ---
    pop_size = 180
    above_thresh_count = 47

    # --- Workbook ---
    wb = openpyxl.Workbook()
    ws1 = wb.active
    ws1.title = "Journal Entries"
    h1 = ["JE_Number", "Posting_Date", "Account", "Description", "Amount_GBP", "Preparer", "Approver", "Period", "Supporting_Doc"]
    for c, h in enumerate(h1, 1):
        ws1.cell(row=1, column=c, value=h)
    _hdr(ws1, 1, len(h1))
    ...
    ws2 = wb.create_sheet("Sampling")
    ws2.cell(row=1, column=1, value="Sampling").font = TITLE_FONT
    ws2.cell(row=3, column=1, value="Population Size").font = BOLD_FONT
    ws2.cell(row=3, column=2, value=str(pop_size)).font = NORM_FONT
    ws2.cell(row=4, column=1, value="Above Threshold (GBP 100K)").font = BOLD_FONT
    ws2.cell(row=4, column=2, value=str(above_thresh_count)).font = NORM_FONT
    ws2.cell(row=5, column=1, value="Sample Size").font = BOLD_FONT
    ws2.cell(row=5, column=2, value=str(len(sample))).font = NORM_FONT
    ws2.cell(row=6, column=1, value="Method").font = BOLD_FONT
    ws2.cell(row=6, column=2, value="Stratified random (3 above + 2 below threshold)").font = NORM_FONT
    ws2.cell(row=8, column=1, value="Selected Sample").font = TITLE_FONT
    sh = ["Sample_No", "JE_Number", "Amount_GBP", "Preparer", "Approver"]
    for c, h in enumerate(sh, 1):
        ws2.cell(row=9, column=c, value=h)
    _hdr(ws2, 9, len(sh))
    for i, s in enumerate(sample):
        ws2.cell(row=10 + i, column=1, value=i + 1)
        ws2.cell(row=10 + i, column=2, value=s["JE_Number"])
        ws2.cell(row=10 + i, column=3, value=s["Amount_GBP"])
        ws2.cell(row=10 + i, column=4, value=s["Preparer"])
        ws2.cell(row=10 + i, column=5, value=s["Approver"])
    ...
    # --- Emails that reference the EXACT sample items ---
    ...
    _write_eml(evi / "approval_email_001.eml",
        ...
        "Re: JE Approval Request: JE-2024-0015 - GBP 245,000 - APPROVED",
        (
            "Hi Alice,\n\n"
            "JE-2024-0015 is approved. I have reviewed the accrual schedule (ACC-2024-015) "
            "and confirm the amount of GBP 245,000 to 4100-Sales Revenue is appropriate.\n\n"
            "As this is above the GBP 100K threshold, I am signing off as Finance Director "
            "per GSK-FIN-POL-042.\n\n"
            "Please proceed with posting to Period P01.\n\n"
            "Regards,\nSarah Mitchell\nFinance Director"
        ),
    ...
    _write_eml(evi / "approval_email_002.eml",
        ...
        (
            "Journal entry JE-2024-0043 has been approved.\n\n"
            "JE Number: JE-2024-0043\n"
            "Amount: GBP 187,500\n"
            "Account: 6100-SGA Expense\n"
            "Description: Reclassification of marketing costs\n"
            "Preparer: Bob Carter\n"
            "Approver: Bob Carter\n\n"
            "** WARNING: This is a SELF-APPROVAL. The preparer and approver are the same "
            "individual, which violates the dual authorization requirement under "
            "GSK-FIN-POL-042. This entry also exceeds the GBP 100K threshold and should "
            "have Finance Director review. **"
        ),
        "Wed, 20 Mar 2024 14:00:00 +0000")

    _write_eml(evi / "approval_email_003.eml",
        "david.evans@gsk.com", "emily.foster@gsk.com",
        "Re: JE Approval Request: JE-2024-0112 - GBP 156,000 - APPROVED",
        (
            "Hi David,\n\n"
            "JE-2024-0112 is approved. I reviewed the correction memo (COR-2024-112). "
            "The R&D misposting has been verified and the correction is appropriate.\n\n"
            "Amount: GBP 156,000 to 7200-R&D Expense.\n"
            "Above-threshold Finance Director sign-off: Emily Foster.\n\n"
            "Note: This entry was posted to Period P09 but the expense relates to P10 "
            "activity. Please confirm the posting period is correct.\n\n"
            "Regards,\nEmily Foster\nFinance Director"
        ),
        "Sat, 05 Oct 2024 16:30:00 +0000")

    # --- JE Listing PDF with per-sample detail ---
    pdf = StyledPDF("Journal Entry Listing")
    pdf.alias_nb_pages()
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 14)
    pdf.cell(0, 10, "Manual Journal Entry Listing - FY2024", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 9)
    pdf.cell(0, 6, f"Total Population: {pop_size} entries | Above GBP 100K: {above_thresh_count}", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 6, f"Selected Sample: {len(sample)} entries (stratified random)", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(4)
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(0, 8, "Sample Item Detail:", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(2)

    for i, s in enumerate(sample):
        pdf.set_font("Helvetica", "B", 9)
        pdf.cell(0, 5, f"--- Sample {i+1}: {s['JE_Number']} ---", new_x="LMARGIN", new_y="NEXT")
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(0, 5, f"  Amount: GBP {s['Amount_GBP']:,.2f}", new_x="LMARGIN", new_y="NEXT")
        pdf.cell(0, 5, f"  Account: {s['Account']}", new_x="LMARGIN", new_y="NEXT")
        pdf.cell(0, 5, f"  Description: {s['Description']}", new_x="LMARGIN", new_y="NEXT")
        pdf.cell(0, 5, f"  Preparer: {s['Preparer']} | Approver: {s['Approver']}", new_x="LMARGIN", new_y="NEXT")
        pdf.cell(0, 5, f"  Posting Date: {s['Posting_Date']} | Period: {s['Period']}", new_x="LMARGIN", new_y="NEXT")
        sup = s['Supporting_Doc'] or "[NOT ATTACHED]"
        if not s['Supporting_Doc']:
            pdf.set_text_color(180, 0, 0)
            pdf.cell(0, 5, f"  Supporting Doc: {sup} ** EXCEPTION **", new_x="LMARGIN", new_y="NEXT")
            pdf.set_text_color(0, 0, 0)
        else:
            pdf.cell(0, 5, f"  Supporting Doc: {sup}", new_x="LMARGIN", new_y="NEXT")

        above = s['Amount_GBP'] >= 100000
        pdf.cell(0, 5, f"  Threshold Status: {'Above GBP 100K - Finance Director review required' if above else 'Below threshold'}", new_x="LMARGIN", new_y="NEXT")

        if s['Preparer'] == s['Approver']:
            pdf.set_text_color(180, 0, 0)
            pdf.cell(0, 5, f"  ** DUAL AUTH EXCEPTION: Preparer = Approver ({s['Preparer']}) **", new_x="LMARGIN", new_y="NEXT")
            pdf.set_text_color(0, 0, 0)

        if s['JE_Number'] == 'JE-2024-0112':
            pdf.set_text_color(180, 0, 0)
            pdf.cell(0, 5, "  ** PERIOD EXCEPTION: Posted to P09 but relates to P10 activity **", new_x="LMARGIN", new_y="NEXT")
            pdf.set_text_color(0, 0, 0)
    ...
    pdf2.set_font("Helvetica", "", 9)
    pdf2.multi_cell(0, 5, textwrap.dedent("""\
        1. THRESHOLD: All manual journal entries above GBP 100,000 require Finance Director review and sign-off.
        2. DUAL AUTHORIZATION: The preparer and approver of a journal entry must be different individuals. Self-approval is strictly prohibited.
        3. POSTING WINDOW: Journal entries must be posted within 5 business days of the relevant period end.
        4. SUPPORTING DOCUMENTATION: Every manual journal entry must have supporting documentation attached (memo, schedule, or reference).
        5. REVIEW EVIDENCE: Approval evidence must be retained (email confirmation or system workflow approval).
    """))
    pdf2.output(str(evi / "je_policy.pdf"))

    print(f"  FIN-042: {pop_size} population, {len(sample)} sample, 3 emails + 2 PDFs")
...
def gen_rev_019():
    ...
    sample = random.sample(orders, min(5, len(orders)))
...
def gen_env_007():
    ...
    # Tab 2: Sampling
    sample = random.sample(inspections, min(5, len(inspections)))
    ws2 = wb.create_sheet("Sampling")
    ws2.cell(row=1, column=1, value="Sampling").font = TITLE_FONT
    ws2.cell(row=3, column=1, value="Population").font = BOLD_FONT
